# KEPUTUSAN OWNER — 2026-06-13 (menjawab seluruh OPEN QUESTIONS audit V3)
**Sumber:** wawancara owner 2026-06-13 (Liem Lui) menjawab pertanyaan audit forensik & OPEN QUESTIONS. Dokumen ini MENGIKAT — bila konflik, dokumen ini menang. Detail temuan & task per-keputusan kini ada di **dossier domain `10`-`19`**; peta eksekusi (fase) di `00_BLUEPRINT.md §4`. Keputusan diberi ID per bagian (D/R/B/C/E/F/G/H/I/J/K/L/M/N/O/P/Q).
...
## D — KEPUTUSAN UTAMA (D-01 s/d D-16)
...
| D-03 | **DRAFT invoice TIDAK memblokir checkout.** Saat forced checkout, exclude + auto-cancel DRAFT (jangan biarkan 1 draft terlupakan = overstay tak pernah selesai). | Task F3-13. |
| D-04 | **Expiry booking = 3 JAM FLAT semua jalur** (bukan cutoff 21:00 WIB). Booking malam berlaku 3 jam berikutnya. | Task F1-11. |
| D-05 | **Admin tidak boleh ubah deposit.** Deposit jaminan SELALU = `Room.defaultDepositRupiah` (C3). | Task F1-10. |
| D-06 | **DATABASE MASIH TESTING → deploy FRESH.** Lihat bagian atas. | ⬆️ |
...
| D-13 | **Keluar lebih awal: sewa HANGUS, deposit kembali normal.** (K-e) | Pertahankan. |
...
| ID | Keputusan | Dampak |
|----|-----------|--------|
| R1 | Tenant lama punya **prioritas eksklusif sampai hari-H tanpa wajib DP dulu** (L2). | F2-1 state machine. |
...
- K-1: resolved time dari assignedAt → F3-3
- K-6/K-8: notif penerima salah → F3-2
