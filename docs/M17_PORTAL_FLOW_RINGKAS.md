...
Status: SEBAGIAN — Iterasi 1 selesai (DashboardAdmin), Iterasi 2–4 belum.
...
- [ ] **Iterasi 2** — Kartu tugas + route terfilter (query param `status`/`ktpStatus`/`tab`) dipastikan di semua halaman tujuan.
- [ ] **Iterasi 3** — OwnerDashboard ringkas (KPI + prioritas) sesuai rancangan.
- [ ] **Iterasi 4** — Uji flow end-to-end (booking → bayar → perpanjangan → checkout) dan cleanup widget lama.
