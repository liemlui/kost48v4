...
## 2026-05-28 — M3.2 FULL First-Paid Runtime UAT

### Changed

- Updated M3.2 status from `PASS with BR5 partial coverage` to `FULL PASS`.
- Locked BR5 first-paid-wins pure API-only runtime evidence.
- Documented that valid admin-approved payment cancels competing unpaid booking.
- Documented targeted cleanup result for BR5 test data.

### Verified

```text
Stay A = 18 ACTIVE
PaymentSubmission A = 6 APPROVED
Invoice A = 30 PAID
Room G2-004 = OCCUPIED with activeStayId=18 during winner state
Stay B = 19 CANCELLED
Room G2-004 cleanup = AVAILABLE, activeStayId=null, currentStay=null
```

### Not changed

- No backend code hotfix.
- No frontend code hotfix.
- No DB reset.
- No production mutation.
- No SQL fallback for artificial AutoOps competing scenario.
