...
**File inti:** `accounting-posting.service.ts` (22.4KB — jurnal posting), `accounting-report.service.ts` (10.6KB), `accounting-readiness.service.ts`.
...
| Reports (P&L, Balance Sheet, Cashflow) | `accounting-report.service.ts` |
| General Ledger query | Prisma raw + report helpers |

## 3. Temuan audit
| ID | Sev | Dampak bisnis | Lokasi | Fix/Task |
|---|---|---|---|---|
| F-01 | 🔴 P1 | Cashflow P&L detected cash as AR — prefix `11` (piutang) masuk kategori kas → laporan cashflow overstate. | `accounting-report.service.ts` | **F1-3**: perbaiki mapping prefix |
...
| F-10 | 🟠 P2 | **Cashflow report menghitung DEPOSIT sebagai cash inflow** → DANA TITIPAN (liability) masuk kas operasional, menyesatkan pemilik bisnis tentang uang yang sebenarnya BISA DIPAKAI. Harusnya: deposit di EXCLUDE dari operating cashflow, ditampilkan di section terpisah (financing/liability). Owner: deposit adalah dana titipan (D1/D2). | `accounting-report.service.ts` cashflow calculation | **F1-9**: exclude deposit dari operating cashflow, pisahkan ke section "Perubahan Liabilitas Titipan" |
| F-17 | 🟠 P2 | Balance Sheet: total aset ≠ liabilitas + ekuitas saat ada jurnal partial → imbalance karena mapping akun tidak lengkap. | `accounting-report.service.ts` BS query | **F1-5**: perbaiki grouping akun balance sheet |
...
- **F2-8 · FASE 2:** hardening readiness gate untuk auto-close bulanan.
...
**Lintas-dossier:** jurnal booking/payment → dossier 10; jurnal deposit → dossier 12; COA & readiness → dossier 03.