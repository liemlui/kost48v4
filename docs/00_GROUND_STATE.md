...
**Status:** Source of truth utama setelah M3.2 FULL First-Paid Runtime UAT. Latest pushed code commit 296bd8d. BR5 pure API-only PASS; next recommended phase M4 Deposit Ledger Detail PLAN.
...
## 0.1 Ground State After M3.2 FULL PASS

```text
KOST48 V5 current operational runtime gate:
- Tenant Guide and AutoOps Control UX are shipped.
- First-paid room priority is runtime verified.
- AutoOps safe deterministic cleanup is runtime verified.
- Sensitive flows remain human/core controlled.
```

Current product confidence:
- Room priority business rule is verified for pure unpaid competitor.
- Booking without payment does not win against approved payment.
- Payment pending review remains protected from auto-cancel.
- Rejected/expired unpaid bookings are clean-up eligible.
- Room can be released from orphan RESERVED state.
- No new code patch is needed before M4 planning.

Next source-of-truth direction:
```text
M4 Deposit Ledger Detail should improve financial/lifecycle auditability while preserving the existing monolith lifecycle rules.
```
