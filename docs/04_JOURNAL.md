...
## 2026-05-28 — BR5 Pure Runtime UAT Journal
...
- Stay B = 19
- Tenant B = 20
- No payment submitted for B.
```

Observed:
- Booking A was approved and invoice `30` issued.
- Booking B was created for the same room without payment.
- Tenant A submitted payment `6` with `TRANSFER`.
- Admin approved payment `6`.
- Payment `6` became `APPROVED`.
- Invoice `30` became `PAID`.
- Stay A `18` remained `ACTIVE`.
- Room `G2-004` became `OCCUPIED` with activeStayId `18`.
- Stay B `19` became `CANCELLED`.
- Cancel reason confirmed first-paid priority:
  `Kamar sudah diamankan oleh pembayaran tenant lain. Prioritas kamar mengikuti pembayaran valid pertama.`

Cleanup:
```text
Targeted cleanup SQL executed successfully.
Room 4 / G2-004 verified after cleanup:
- status = AVAILABLE
- activeStayId = null
- currentStay = null
...
Conclusion:
```text
BR5 First-Paid-Wins Pure Runtime UAT = PASS.
M3.2 is now FULL PASS.
```
