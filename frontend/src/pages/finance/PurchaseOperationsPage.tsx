  ...
  return <FeatureErrorBoundary><div className="purchase-operations-page">
    ...
    <Modal show={showEntry} onHide={() => !saveMutation.isPending && setShowEntry(false)} centered size="lg" scrollable><Form onSubmit={submit}>
      ...
      <Modal.Body>{error ? <Alert variant="danger">{error}</Alert> : null}<Alert variant={form.createAssetCandidate ? 'warning' : 'info'} className="small">{form.createAssetCandidate ? 'Pengeluaran akan dicatat sebagai biaya. Sistem juga membuat kandidat aset berstatus draft untuk direview; jurnal aset tidak dibuat otomatis.' : 'Saat disimpan, pengeluaran langsung dikonfirmasi dan sistem mencoba membuat jurnal sesuai konfigurasi accounting.'}</Alert>
        <Row className="g-3"><Col md={4}><Form.Group controlId="purchase-operations-page-1"><Form.Label>Tanggal beli / bayar</Form.Label><Form.Control required type="date" value={form.expenseDate} onChange={(event) => setForm((current) => ({ ...current, expenseDate: event.target.value }))} /></Form.Group></Col><Col md={4}><Form.Group controlId="purchase-operations-page-2"><Form.Label>Masuk ke</Form.Label><Form.Select value={form.category} onChange={(event) => { const category = event.target.value as ExpenseCategory; setForm((current) => ({ ...current, category, type: category === 'INTERNET' || category === 'RENT_BUILDING' || category === 'SALARY' ? 'FIXED' : current.type })); }}>{(Object.keys(CATEGORY_META) as ExpenseCategory[]).map((category) => <option key={category} value={category}>{CATEGORY_META[category].icon} {CATEGORY_META[category].label}</option>)}</Form.Select></Form.Group></Col><Col md={4}><Form.Group controlId="purchase-operations-page-3"><Form.Label>Sifat biaya</Form.Label><Form.Select value={form.type} onChange={(event) => setForm((current) => ({ ...current, type: event.target.value as PurchaseForm['type'] }))}><option value="VARIABLE">Variabel</option><option value="FIXED">Tetap</option></Form.Select></Form.Group></Col><Col md={8}><Form.Group controlId="purchase-operations-page-4"><Form.Label>Barang / keterangan pembayaran</Form.Label><Form.Control required value={form.description} placeholder="Contoh: Token listrik PLN Juli 2026" onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))} /></Form.Group></Col><Col md={4}><Form.Group controlId="purchase-operations-page-5"><Form.Label>Nominal</Form.Label><CurrencyInput required value={form.amountRupiah} onChange={(amountRupiah) => setForm((current) => ({ ...current, amountRupiah }))} placeholder="0" /></Form.Group></Col><Col md={6}><Form.Group controlId="purchase-operations-page-6"><Form.Label>Vendor / tempat beli</Form.Label><Form.Control value={form.vendorName} placeholder="PLN, Tokopedia, toko, teknisi" onChange={(event) => setForm((current) => ({ ...current, vendorName: event.target.value }))} /></Form.Group></Col><Col md={6}><Form.Group controlId="purchase-operations-page-7"><Form.Label>Catatan</Form.Label><Form.Control value={form.note} placeholder="Nomor meter, invoice, atau detail barang" onChange={(event) => setForm((current) => ({ ...current, note: event.target.value }))} /></Form.Group></Col></Row>
        ...
        {form.createAssetCandidate ? <Row className="g-3 mt-1 p-2 rounded border bg-light"><Col md={6}><Form.Group controlId="purchase-operations-page-8"><Form.Label>Kategori kandidat aset</Form.Label><Form.Select value={form.assetCategory} onChange={(event) => setForm((current) => ({ ...current, assetCategory: event.target.value as FixedAssetCategory }))}>{(Object.keys(ASSET_CATEGORY_LABELS) as FixedAssetCategory[]).map((category) => <option key={category} value={category}>{ASSET_CATEGORY_LABELS[category]}</option>)}</Form.Select></Form.Group></Col><Col md={3}><Form.Group controlId="purchase-operations-page-9"><Form.Label>Masa manfaat</Form.Label><Form.Control type="number" min={1} value={form.usefulLifeMonths} onChange={(event) => setForm((current) => ({ ...current, usefulLifeMonths: Number(event.target.value) || 1 }))} /><Form.Text>bulan</Form.Text></Form.Group></Col><Col md={3} className="d-flex align-items-end"><Form.Check type="switch" id="purchase-depreciation" checked={form.depreciationEnabled} onChange={(event) => setForm((current) => ({ ...current, depreciationEnabled: event.target.checked }))} label="Siap disusutkan" /></Col></Row> : null}
