...
export function useBusinessHealthScore(input: BusinessHealthInput): BusinessHealthScore {
  return useMemo(() => {
    ...
    const assistantItems: AssistantItem[] = dedupeCommandItems([
      ...
      ...(approvedCheckoutRequestCount ? [{ id: 'owner-checkout-health', ruleId: 'checkout-final', entityType: 'checkout', entityId: 'summary', dedupKey: 'checkout-final|summary|/stays?status=BOOKINGS', severity: 'HIGH' as const, title: 'Checkout disetujui belum final', message: 'Pastikan tagihan dan deposit clear sebelum kamar dilepas kembali.', count: approvedCheckoutRequestCount, source: 'Checkout', actionLabel: 'Pantau', actionTo: '/stays?status=BOOKINGS' }] : []),
      ...(pendingRenewCount ? [{ id: 'owner-renew-health', ruleId: 'renew-pending', entityType: 'renew', entityId: 'summary', dedupKey: 'renew-pending|summary|/renew-requests', severity: 'MEDIUM' as const, title: 'Perpanjangan perlu keputusan', message: 'Keputusan renew menjaga okupansi dan memicu invoice renewal.', count: pendingRenewCount, source: 'Renew', actionLabel: 'Review renew', actionTo: '/renew-requests' }] : []),
