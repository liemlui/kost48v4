...
export function useStay(id?: number | string) {
  ...
  const invalidate = async () => {
    ...
    await queryClient.invalidateQueries({ queryKey: ["payment-submissions"] });
    await queryClient.invalidateQueries({
      queryKey: ["deposit-ledger", "stay", id],
    });
    await queryClient.invalidateQueries({
      queryKey: ["deposit-ledger", "summary"],
    });
    await queryClient.invalidateQueries({
      queryKey: ["deposit-ledger", "reconciliation-lite"],
    });
    await queryClient.invalidateQueries({
      predicate: (query) => {
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: Partial<Stay>) =>
      updateStay(id as number | string, payload),
    onSuccess: invalidate,
  });

  const completeMutation = useMutation({
    mutationFn: (payload: CompleteStayPayload) =>
      completeStay(id as number | string, payload),
    onSuccess: invalidate,
  });

  const processDepositMutation = useMutation({
    mutationFn: (payload: ProcessDepositPayload) =>
      processDeposit(id as number | string, payload),
    onSuccess: invalidate,
  });

  const cancelMutation = useMutation({
    mutationFn: (payload: CancelStayPayload) =>
      cancelStay(id as number | string, payload),
    onSuccess: invalidate,
  });
