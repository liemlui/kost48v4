...
## Ringkasan
Batch patch backend ini menyelesaikan slice **V4 Fase 4.1A backend-only** untuk **admin booking approval core**, dengan fokus pada endpoint yang diminta:
- `PATCH /admin/bookings/:stayId/approve`

Pendekatan implementasi dijaga tetap sempit: **tidak membuka frontend 4.1B, tidak membuka payment submission, tidak mengubah room menjadi `OCCUPIED`, tidak membuka scheduler umum, debt flow, redesign accounting, atau fase 4.2+**.

## Scope Patch Batch Ini
1. **Endpoint approval admin**
   - Menambahkan `PATCH /admin/bookings/:stayId/approve`
   - Hanya untuk role `OWNER` dan `ADMIN`

2. **DTO approval booking**
   - `agreedRentAmountRupiah`
   - `depositAmountRupiah`
   - `initialElectricityKwh`
   - `initialWaterM3`

3. **Transaksi atomik approval booking**
   - Lock booking + room di dalam transaksi
   - Validasi booking reserved aktif, tenant aktif, room aktif, `bookingSource = WEBSITE`, belum punya invoice awal, dan belum kedaluwarsa
   - Buat 2 baseline `MeterReading`
   - Buat invoice awal `DRAFT` + minimal 1 line `RENT`
   - Audit log approval + create invoice tetap tercatat

4. **Honesty terhadap fase aktif**
   - `Room.status` tetap `RESERVED`
   - Invoice tetap `DRAFT`
   - Tidak membuka payment approval atau aktivasi penuh hunian
...
### Dibuat
- `src/modules/tenant-bookings/admin-bookings.controller.ts`
- `src/modules/tenant-bookings/dto/approve-booking.dto.ts`
...
Dicoba tetapi gagal karena environment:
- `npx prisma generate`
  - gagal mengakses `binaries.prisma.sh`
...
## Hotfix Tambahan Pasca Audit Final
- Surface **Fase 4.1** yang sempat ikut terbuka sebelumnya ditutup kembali agar selaras dengan source of truth proyek.
- Endpoint `PATCH /admin/bookings/:stayId/approve` **tidak lagi diekspos** pada batch final ini.
- Fokus backend kembali sempit di Fase 4.0: `GET /public/rooms`, `POST /tenant/bookings`, dan `GET /tenant/bookings/my`.
