...
export class AutoOpsService implements OnModuleInit, OnModuleDestroy {
  ...
  private async expireRenewalPriorityTx(
  ...
  private async notifyTenantRenewalExpired(stayId: number, tenantId: number) {
    const tenantUser = await this.prisma.user.findFirst({
  ...
  private async notifyAdminsRenewalForfeited(requestId: number, stayId: number) {
